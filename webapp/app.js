...
const API = {
  ...
  // Income definitions (templates): { id, name, amount }. Each is posted as a
  // plain transaction at the start of every month; edits/deletes only affect
  // months not yet posted.
  async income() {
    const r = await fetch('/api/income', { headers: this.headers() });
    if (!r.ok) throw new Error(`Load failed (${r.status})`);
    return (await r.json()).incomes;
  },
  // Create an income definition. Returns { income, transaction } — the transaction
  // is the instance posted for the current month, to drop into the month cache.
  async incomeAdd(name, amount) {
    const r = await fetch('/api/income', {
      method: 'POST', headers: this.headers(), body: JSON.stringify({ name, amount }),
    });
    if (!r.ok) throw new Error(await detail(r) || `Add failed (${r.status})`);
    return await r.json();
  },
  async incomeEdit(id, patch) {
    const r = await fetch(`/api/income/${id}`, {
      method: 'PATCH', headers: this.headers(), body: JSON.stringify(patch),
    });
    if (!r.ok) throw new Error(await detail(r) || `Edit failed (${r.status})`);
    return (await r.json()).income;
  },
  async incomeRemove(id) {
    const r = await fetch(`/api/income/${id}`, {
      method: 'DELETE', headers: this.headers(),
    });
    if (!r.ok) throw new Error(`Delete failed (${r.status})`);
...
async function onEditDelete() {
  ...
  try {
    // Income is a definition (its own endpoint); a recurring expense is a transaction.
    // Deleting an income only stops future months — already-posted instances remain.
    if (isIncome) await API.incomeRemove(id);
    else await API.remove(id, (STATE.editingTx.date || '').slice(0, 7));
    STATE.editingId = null; STATE.editingTx = null;
...
function addForm(type) {
  ...
  if (type === 'income') {
    ...
    return `
      ...
      <div class="field"><label>Amount</label><div class="amount-input"><span>$</span><input id="f-amount" type="number" inputmode="decimal" placeholder="0.00" step="0.01" min="0" value="${amtVal}"></div></div>
      <p class="hint-text">${e ? 'Changes apply from next month onward. This month and past months keep the amount already recorded.' : 'Recurring monthly income — recorded automatically at the start of every month.'}</p>
...
async function onAddSubmit() {
  ...
  // Editing an existing income definition → PATCH the definition. Only months not
  // yet posted pick up the change; the current and past months keep their instances.
  if (type === 'income' && STATE.editingId) {
    if (btn) { btn.disabled = true; btn.textContent = 'Saving…'; }
    try {
      await API.incomeEdit(STATE.editingId, { name, amount });
      STATE.editingId = null; STATE.editingTx = null;
      STATE.income = null;   // reload the list
      invalidateBudget();
      haptic('success');
      toast('✅ Updated — applies next month');
      renderAdd();
    } catch (e) {
      haptic('error');
      toast('⚠️ ' + (e.message || 'Could not save'));
  ...
  // New income → create a definition; the backend also posts this month's instance.
  // Later months are generated by the monthly job.
  if (type === 'income') {
    if (btn) { btn.disabled = true; btn.textContent = 'Saving…'; }
    try {
      const { transaction } = await API.incomeAdd(name, amount);
      const ym = (transaction.date || '').slice(0, 7);
      if (STATE.cache[ym]) STATE.cache[ym].push(transaction); else invalidate(ym);
      invalidateBudget();
      STATE.income = null;   // reload the definitions list
      haptic('success');
      ...
      haptic('error');
      toast('⚠️ ' + (e.message || 'Could not save'));
      if (btn) { btn.disabled = false; btn.textContent = 'Add income'; }
    }
    return;
  }
